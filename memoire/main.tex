...
let list_iter ($\textcolor{black}{\text{f:int -> unit}}$) (l: int list)
 = let rec loop (ai: int list)   
   ...
   in loop l  
 	\end{whycode}
 \end{small}	 
How do we represent the invariant of \texttt{list\_iter} ? As we see, the iteration on list \texttt{ai} relies on the structure of lists itself, using recursion and pattern-matching: at each step, we call \texttt{list\_array} recursively on the sub-list \texttt{xs} of \texttt{ai}.
...
However, \texttt{l} and \texttt{bi} are not \textit{structurally} the same: the order of elements has been \textbf{reversed}. We must take that into account, when we apply the invariant \texttt{inv: int list -> int list -> prop} on $bi$ and $l$. Consequently, the specification of \texttt{list\_iter} should be: 
...
As we can see, the original order of elements is restored in specification of \texttt{list\_iter} by reversing the entire list \texttt{l}; in specification of \texttt{loop} by concatenating the accessed elements \texttt{bi} in the reversed order to list \texttt{ai} of the remaining elements of \texttt{l}.

The annotated program \texttt{sum\_iter} using \texttt{list\_iter} becomes:
\begin{whycode}
...
To prove the verifications conditions above, we need various lemmas about logical functions of Why3 standard library such as \texttt{rev}, \texttt{rev\_append}. We also need write prove the following lemmas about function $\sum$ itself (defined as recursive function by induction on list l):   
\begin{small}
\begin{whycode}
