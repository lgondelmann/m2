...
\subsubsection*{\texorpdfstring{$Ghost-MiniML$}{$Ghost-MiniML$} Syntax and Semantics}

\subsubsection*{\texorpdfstring{$Ghost-MiniML$}{$Ghost-MiniML$} Language  Typing Relation}
...
\end{figure} 
\subsubsection*{Erasure of Ghost-Code}
\subsubsection*{Properties of Erasure}
%In this example, the invariant parameter \texttt{inv} appears only inside specification, so we can be sure that it does not modify the meaning of code itself. 
%
%However, in the case of iterators on algebraic data types such as lists and trees, additional parameters that we need use to express the invariant of iterator do appear inside the source code
%, and it becomes less clear that the semantics of the source code is preserced.
%	
