...
\begin{array}{l@{\hspace*{1cm}}l@{\hspace*{1cm}}l}
	...
	(2)& \typerule{t'}{\tau}{\theta}{\rho} &\textsc{(typing and effects preservation)} \\
	(3)& t' \in \inlT & \textsc{(A-normal form preservation)} \vspace*{-1cm}
\end{array}
\end{displaymath}
 \label{inllet-prop-l}
\end{lemma}
...
$$\textit{(t~F) \qquad r~:=~F \qquad if~F~then~... else ... \qquad match~F~with~...}$$
	Finally, from theorem hypotheses it follows that $\typerule{s1}{\tau^2}{\bth}{\brh}$.	
	Therefore, by \cref{preserv-prop2-d}, (3) holds. 
 \end{proof}
