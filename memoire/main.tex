...
%
%	This memoir is a study about how high-order definitions can be used in the context of deductive software verification, when the available specification environment is limited to first-order logic. 
%			
%	Nevertheless, evaluation of large-scale functional programming corpora shows that most of higher-order programs encountered in practice 
%	are essentially equivalent to using a simple imperative loop construction.
%	
%	belong to a restrained class of second-order expressions 
%	
%	
%	such as \textit{iterators} \textit{mappings}, \textit{folders} for which verification which are equivalent simple imperative loop.
%  The goal of this memoir is to present the 	

%	
%	
%	Concretely, this work presents a technical solution of higher-order programs \textit{inlining} and provides its \textit{correctness} proof. 
%	Evaluation of higher-order expressions inside large-scale corpora shows that without being complete, this solution should fit most of the higher-order programs OCaml developers may encounter in practice. 
...
%	
	We begin this work by an informal presentation of the context, the state of art and this memoir main ideas. 
	

\subsubsection*{The Context of Deductive Program Verification}
...
or array bounds check, occur ubiquitously in any realistic software
 verification process,but were not supported by the earlier models of automated theorems provers, leaving no choice but to use proof assistants.
...
\subsubsection*{An Introductory Example}		
	...
	Does all this mean that, as long as ATPs remain first-order, proving a piece of higher-order functional code, no matter how simple the code is, should require more sophisticated or less automated tools ? 
	
	In one of his blog posts\footnote{\meganerd}, Eric di Castro Lopo, author of widely-used C-library \textit{libsndfile}\footnote{\libsndfile}, gives the following example of two programs, one using a iterative loop, and the other using iterator functional, that compute a sum of the elements of an \textit{array}\footnote{we slightly adapt his examples to the syntax of WhyML} \\
\begin{small}
...
\end{small}
	...
	As Linux programmer states, functions like \texttt{iter}, \texttt{map}, or \texttt{fold}: \textit{``can reduce the number of points of possible error in a program. More importantly, for the code reader who understands and is comfortable with these techniques, reading and understanding code using these functions is quicker than reading and understanding the equivalent for loop.''}
			
	So what if such a reader wants to verify of code correctness formally? Is there anything similar to \textit{Hoare logic} rule for \texttt{while} for \texttt{Array.iter}? Could we do verification of the latter as easily as for the while loop ? 
	To make the comparison clear, let us first illustrate how the sum of array elements computed using \textit{while} loop is specified and proved in Why3.  

To prove that \texttt{sum\_loop} computes $\sum_{~0\leq i < n} a[i]$:
\begin{footnotesize}
	\begin{whycode}  
     let sum_loop (a: array int) 	
       $\textcolor{OliveGreen}{\text{requires \{ true \}}}$
       $\textcolor{OliveGreen}{\text{ensures}\{\sum_{~0 \leq i < n} a[i]\}}$ = ...
 	\end{whycode}
 \end{footnotesize}
\
we typically introduce a \textit{loop invariant}. Here, the invariant is:
\ 
 \begin{footnotesize}
 	\begin{whycode}  
     while !i < n do $\textcolor{OliveGreen}{\text{invariant sum } = \sum_{~0\leq j < i} a[j] \bwedge 0 \leq !i \leq n }$ ...
 	\end{whycode} 
\end{footnotesize}  
\
The verification conditions we get from these specifications are:
\begin{footnotesize}
\begin{displaymath}
...
(vc2)
	& (0 \leq 0 \bwedge 0 \leq n) 
\bwedge 0 = \sum_{~0\leq j < 0} a[j]	
	& \textsc{(initialisation)} \\


(vc3)
	& \forall i. (0 \leq i < n \bwedge !s = \sum_{~0 \leq j \leq i} a[j]) \brarr
		(0 \leq (i+1) \leq n) 
		\bwedge !s = \sum_{~0 \leq j \leq i+1} a[j]
	& \textsc{(preservation)} \\
...
All verification conditions above are elementary first-order formulas with linear arithmetic and can proved by Alt-Ergo SMT solver instantly. So can we do \textit{exactly} the same thing with \texttt{Array.iter} ? In particularly, we would like to get verifications conditions that should be roughly the same. 

\subsubsection*{A Technical Solution: Inlining}

	One possible solution is higher-order program \textit{inlining}.
...
each higher-order expression into an expression of first-order degree. 
	Local variables of a higher-order type are replaced by functional expression they introduce; 
...
type, remains syntactically unchanged. 
	...
	\begin{small}
	\begin{whycode}  
  let array_iter f a $\textcolor{OliveGreen}{inv}$ =
    let rec loop i =  	
      $\textcolor{OliveGreen}{\text{requires \{ \text{ inv } i \}}}$
      $\textcolor{OliveGreen}{\text{ensures}\{ \text{ inv (length a)}\}}$ 
...
\section{Mini-ML: Programming Language}
	Within the deductive software verification approach, the first step is to write a formal specification for a given program. 
	Programs are usually written in some \textit{intermediate specification language}, which corresponds to a fragment of some existing programming language enriched with logic assertions and other logical formulae used to write specifications. 
	...
	For instance, the computational component of framaC TODO is a 100 per cent compatible with C-language. 
	     
	In this section we present briefly the \textit{Mini-ML} programming language. 	Mini-ML corresponds to a tiny fragment of \textit{WhyML}'s TODO computational part.	
	Roughly speaking it is equivalent to \textbf{PCF + state}. 
...
\subsection{Syntax}	
The formal description of Mini-ML syntax is given below: 
...
\noindent	\textbf{Types and Effect indicators}	\indent  
	...
	Arrows are labelled by greek letters $\theta$ and $\rho$, which represent two distinct function's \textit{latent effects}:
...
\noindent	\textbf{Terms, Values and Constants}	\indent  
	Each Mini-ML program consists of two parts. 
	First, all global references are declared and instantiated with some value.
	These declarations are then followed by a term.

	Each term is either a value or a compound construction like application, local binding, etc.
	...
	Note that references, variables, and formal parameters of functions are \textbf{explicitly typed}. 
	Also note that terms are put in \textbf{A-normal} form\footnote{\cite{BartheC02}}: in applications, a term $t$ is always applied to some already value form $v$ where the result of every intermediate computation is named via a \textbf{let} construct.
...
%$\underline{match~v~with~Nil~\rightarrow~t_1~|~Cons~n~l~\rightarrow~t_2}$ 
%TODO match !!!
\subsection{Semantics}
\label{mini-ml-def-sem}
	The Semantics of Mini-ML is given by the following SOS (small-step operational semantics) inference rules where $\mu$ and $\mu'$ are global reference store transition states:	
	\begin{figure}[H]
	\begin{spacing}{1.2} 	
...
\end{spacing}
%\caption{ \textbf{Semantics Rules} \hfill}
\end{figure}
	...
	On the contrary, the rule \textit{E-Head} describes an evaluation step where term's construction is modified "on the top``, passing from one form to another:
\begin{figure}[H]
	\begin{spacing}{2} 
  ...
  \infax[E-Op-$\lambda$]
	...
	{\ghead
		{
			(\mu f:~\tau.~
				\lambda x_{\tau_2}. t~v)\mem}
		{t[
			 x_{\tau_2} \mapsfrom v, 
			 f \mapsfrom  \mu f:~\tau.~\lambda x_{\tau_2}. t]\mem}}
...
\end{figure}	
	...
	Finally, note that the two syntactic details above (explicitly typed variables and A-normal form) are somehow irrelevant for semantics of Mini-ML.
...
varia
 ...
 and Typing	
	...
	by $\theta$ and $\rho$
	To explain what the meaning of these labels, 
	
	
	let us first explain the general 
	Indeed, the Mini-ML typing system we describe in \cref{mini-ml-def-typ}  allow us to keeps track of program side-effects such as reference assignment and recursive function use. 
	That is, the typing $\typing{t}{\tau}{\theta}{\rho}$ is a relation between  term
	...
	avoir some dangerous cases like in the  examples TODO,  
	
	
	These labels are latent effects on the arrow are ind
	




\noindent	\textbf{Typing}	\indent  bla
...
\end{theorem}
	...
	Choosing PCF as programming language corresponds to what is commonly done in computer science research~(TODO cite benj piece Abramsky).  
