...
{\infrule[$I_1$-#1]
	...
	\begin{adjustwidth}{-8em}{0em}
		...
		\inlletrule{App$_1$}{t_1}{t'_1}{(\boldsymbol{t_1}v)}{(\boldsymbol{t'_1}v)}

		\inlletrule{App$_2$}{v}{v'}{(s\boldsymbol{v})}{(s\boldsymbol{v'})}
	
		\inlletrule{Let$_1$}{t_1}{t'_1}
			{\tmlet{\varslash{l}{F}}{\boldsymbol{t_1}}{t_2}}
			{\tmlet{\varslash{l}{F}}{\boldsymbol{t'_1}}{t_2}}
	
		\inlletrule{Let$_2$}{t_2}{t'_2}
...
\end{definition}
	...
	\begin{proof}
	  ...
	  Do the remaining  (\textsc{$I_1$-trans}) case	by strong induction on $n$. 
	\end{proof}
...
\begin{definition}([$\inlletNF$, Normal Forms For $\inlletplus$])
 Define $\inlletNF$, the set of normal forms for $\inlletplus$, as follows:
...
\end{definition}
	...
	\begin{theorem}([Termination of $\inlletplus$]) 
		...
		$$\forall t_0.~ t_0 
	 			\exists t_n \in \inlT: 
	 				(t_0 \in \inlT \bwedge t_0 \not\in \inlS) \brarr 
	 				  (\inllett{t_0}{t_n} \bwedge t_n \in \inlletNF).$$ 

\label{inllet-term-l}.
\end{theorem}
\begin{proof}
For detailed proof see \ref{inllet-term-p}.
\end{proof}
...
\begin{theorem}([Normal Forms For $\inlletarr^\star$]) 
$ \inlletNF = \inlS \cup \{ F \}.$
\label{inllet-nforms-l}
\end{theorem}
\begin{proof}
...
\begin{theorem}([Normal Forms For $\inlletarr^\star$]) = Output language S
