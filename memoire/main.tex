...
There may be several solutions to this problem. We could define inlining, without guaranteeing that semantics of the source program is preserved. 
However, our solution consists in defining a subset of ML programs each of which can be inlined soundly. 
...
\paragraph{Restricting Codomain Type}
	...
	Obviously, if $t$ is a $\inlsrc$ program of some second-order type $\tau^2$, we cannot  transform $t$ into some first-order program. 
	From now on, we suppose that whenever we talk about inlining a higher-order program $t$,$t$ itself is a program of some first-order type.
