...
\section{Inlining}
 ...
 For each of these steps we state and prove that it corresponds to a \textit{deterministic} procedure, that always \textit{terminates}, resulting in an entirely first-order program.
	...
	Using a well-know \textit{logical relations} technique, we conclude this section by giving a proof of inlining \textit{total correctness}, i.e. we prove that for every source program, its inlining and the source program itself are semantically equivalent.\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	Our first goal is to inline second order binders $F,G,...$ inside every incoming $\inlsrc$ program \texttt{P}. 
	Indeed, we can inline \textit{all} second order binders of \texttt{P}, because by definition \texttt{P} is a well-typed closed term of some first-order type. 
  Consequently, higher-order variables can occur in P only under the context of some local binding expression $\tmlet{F}{t_1}{t_2}$.
  
	That is, our goal is to define inlining of second order variables as a procedure \fbox{$\inlletstar : \inlsrc \longrightarrow S$}, where $S \subsetneq \inlsrc $ such that no more second-order variables occur in a program of $S$.
	To acheive this goal, we first define $\hookdownarrow$, inlining in \textit{one step}: 
\begin{definition}[(One Step local bindings Inlining, $\hookdownarrow$)]
	Let $t$ be a term of $\inlsrc$ such that $t \not\in \inlS$. 
  Thus $t$ contains at least one occurence of some variable $F$ introduced by $\tmlet{F}{t_0}{t_1}$ construction.

	In that case, we define one step of inlining, $\inllet{t}{t'}$ by the set of inference rules below:
%\cref{fig:inl-let-d}.
...
{\infrule[$I_1$-#1]
  ...
  \begin{footnotesize}
	...
	\infrule[$I_1$-Match$_1$]
		{ \boldsymbol{t_1 \hookdownarrow t_1'}  \quad s_1 \in S }
...
As we can see, the $\ilarr$ relation consists in the head reduction rule
...
second-local binding take place. Then, the first step of inlining is
