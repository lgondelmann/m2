...
\noindent	\textbf{Semantics}	\indent  
	The Semantics of Mini-ML is given by the standard small-step operational semantics rules of the form \fbox{$t_\mu \rightarrow t'_{\mu'}$} where $\mu$ and $\mu'$ are global reference store transition states:	
	\begin{figure}[H]
	\begin{spacing}{1.2} 	
...
\subsection{Type System with Effects}
	...
	That is, in typing relation \fbox{$\vdash t : \tau, \theta, \rho$}, together with a type $\tau$, a term $t$ is assigned a boolean $\theta$ which indicates whether a $t$ contains some reference assignment $r_{\tau'}:=v$ and an boolean 
