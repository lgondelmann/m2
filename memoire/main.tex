...
after the erasure, we would get the term of Mini-ML. 
...
First of all, if \textit{t} has some dynamic  behaviour that \textit{p} does not even have, then the semantics of \textit{p} is not preserved. So a valid verification of \textit{t} does not imply anymore the validity of verification of \textit{p}:
\begin{figure}[H]
\begin{diagram}
t_{\mu} &	 \rTo^{GhostML} & &  
t'_{\mu'} & \\
\dImplies^{\ebot{t_{\mu}}} & & &   \dImplies_{\ebot{t'_{\mu'}}} & & \\
p_{\ebot{t_{\mu}}} & \textcolor{red}{\nrepeat{10}\cdot \text{ \small MiniML} \nrepeat8\cdot >}   & &  q_{\mu_1}
\end{diagram}
\begin{adjustwidth}{5em}{5em}
