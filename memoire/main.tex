...
\newpage
\section{Higher Order Logic Language}
...
In this example above, we extended the the definition of \texttt{list\_iter} with invariant, \texttt{inv} and list of visited elements of \texttt{l}, \texttt{bi}.

If the former appears only inside the \textit{specification} of \texttt{list\_iter},
the latter appears inside the \textit{source code} of \texttt{list\_iter} itself.

That is, in some way we mixed some specification pieces and the source code. Two important questions arise: 

First, we may want to restore the initial program back from the specification. 
...
Secondly, the verifications conditions generated from the specification above, take into account list parameter \texttt{bi}. 
...
A solution used in some modern verification systems, notably in Why3, is to introduce in programs a way to annotate the additional specifications as program \textit{ghost code}.
...
Additionally, \textit{ghost code} is a traceable annotation: tools in which \textit{ghost code} is used, can verify the \textit{non-interference} of \textit{ghost code} with the rest of the program. \\

In this subsection we formalize the notion of \textit{ghost-code} for the programs of MiniML. Concretely, we define Ghost-ML, a version of MiniML in which one can write ghost code inside programs of Mini-ML.

Using explicit ghost-code annotation and a suitable typing system to control the ghost code \textit{propagation} \textit{non-interference}, we explain how to define ghost-code \textit{erasure} so that the executable-code may be extracted from the specified program.

\subsubsection*{\texorpdfstring{$Ghost-ML$}{$Ghost-ML$}}

\paragraph{Semantics}
To define Ghost-ML, we change the Mini-ML syntax as follows:\\

- we mark explicitly by the ghost indicator $\beta$ every variable, reference,  formal parameter, and domain type: 
$$x_\tau^{\rouge{\beta}},~ \quad !r^{\rouge{\beta}}_{\tau},~ \quad \lambda x_\tau^{\rouge{\beta}}. t,~ \quad {\tau^{\rouge{\beta}} \Rightarrow \tau}...;~$$

- we add a key-word \texttt{\rouge{ghost}} to annotate the ghost-code itself: 
$$\rouge{ghost}~x^{\rouge{\beta}},~~~\rouge{ghost}~(if~v~ then~ t_1~ else~ t_2),~ ...;~$$

The indicator $\gbr$ can be either true (variable or term is ghost) or false (variable or term belongs to the source code).
...
\subsubsection*{\texorpdfstring{$Ghost-MiniML$}{$Ghost-MiniML$} Typing Relation}
...
\dfrac
	...
	{\ghosttyping
		{let~x^{\gbra{2}}_{\tau_1} = t_1~in~t_2}
		{\tau_2}
		{\theta_1 \bvee \theta_2}
		{\rho_1 \bvee \rho_2}
		{\gbra{1} \bvee (\gbran{2} \bwedge \gbra{2}^{\rouge{'}})}
...
\end{figure} 
\subsubsection*{Ghost-Code Erasure}


\subsubsection*{Properties of Erasure}
