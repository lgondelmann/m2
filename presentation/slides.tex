...
%  	\begin{itemize}
%  		\item producing logical assertions for the {\red whole program}
%  		\item generating them {\red automatically} from specification \\[1em] 
%  	\end{itemize}
...
\only<1>{
\hspace*{-1em}
\phantom{{\red Scientific challenge}:~automated proof of {\red HO} programs} \\
\hspace*{-1em}
{\phantom{{\blue This work}:~automated proof of {\blue SO stateful} programs}}} 
\only<2>{
\hspace*{-1em}
\textsf{{\red Scientific challenge}:~automated proof of HO programs} \\
\hspace*{-1em}
{\textsf{{\blue This work}:~automated proof of {\blue SO stateful} programs}}} 
\end{frame}
...
\end{adjustwidth}
\only<1>{
$$\textcolor{black}{\textsf{programs are different, but their meaning is the same:}}$$
$${ \red \texttt{ result =} \hspace*{-1em}\sum_{~0 \leq j < 
\text{a.length}}\hspace*{-1em} a[j]~}$$}
\end{frame}
...
%{\quad Loop invariant}
\frametitle<2>
{\quad {Verification conditions and proof}}
%\frametitle<5->
...
\begin{flushleft}
\only<1>{\vspace*{0.5em}
\OliveGreen{$\bullet$} \texttt{\red sum 0 j a} is an axiomatic FO logical sum \\
\OliveGreen{$\bullet$} \texttt{\blue invariant} is a FO logical assertion} 
\only<2>{\vspace*{0.5em}
\OliveGreen{$\bullet$} we used only first-order formulae \\
\OliveGreen{$\bullet$} generated VCs can be easily proved by ATPs}
\end{flushleft}
\end{frame}
...
%\end{whycode}
%\begin{flushleft}
%\only<1-8>{\textsf{{\blue $\bullet$ program is modular}: there is a call to  \texttt{Array.iter}}}
%\only<1>{\white \textsf{: we need to specify {\texttt{ Array.iter}}}}
%\only<2-8>{\white \textsf{: we need to specify {Array.iter}}\\}
%\only<1-8>{\noindent \textsf{{\blue $\bullet$ specification is modular:} we need to specify iterator}}
%\only<3>{\white \textsf{: we can to prove {\texttt{Array.iter}} too}}
%\only<4-8>{\white \textsf{: we can prove {\red \texttt{Array.iter}}}\\}
%\only<5-8>{\textsf{{\blue modular proof}}:}
%\only<5>{\white\textsf{: \texttt{Array.iter} { body} is { second-order} program}}
%\only<6>{\textsf{ Array.iter is {\red second-order} program}}
%\only<7>{\textsf{ {\red argument \texttt{f}} is a {\red stateful} function}}
%\only<8>{\textsf{ \quad {\red may be difficult to automatize}}\\}
%\end{flushleft}
%\begin{center}
%\vspace*{-2em}
...
\frametitle<5>{Pros and cons}
\frametitle<6>{Solution}
\begin{whycode}  
...
\begin{center}
\texttt{{\red cons}: specification above is {\red partial}}
\texttt{{\blue pros}: specification above is {\blue modular}}\\
\end{center}
\end{minipage}}}
\only<6>{
\hspace*{3.5em}
...
\texttt{\OliveGreen{instantiate} functional arguments}
\end{center}
\end{minipage}}}
\end{frame}
...
\end{flushleft} 
%\only<2>{$$\fbox{\OliveGreen{\textbf{we get a first-order program and specification}}}$$}
%\only<3>{$$\fbox{\OliveGreen{\textbf{generated VCs can be proved automatically}}}$$}
%\end{center}
\end{frame}
...
\frametitle<3>{Effects}
\frametitle<4>{Reference assignment}
\frametitle<5-6>{Recursive calls}
...
\only<3>{
$$\Gamma \vdash t: \tau^{\texttt{\textbf{i}}}{, {\red \theta, \rho}}$$\bigskip
$$ \textsf{\red t may produce side-effects} $$
$$~$$
$$~$$}
\only<4>{
$$\hspace*{1em} \Gamma \vdash t: \tau^\texttt{i}, {\red \theta}, \rho $$ \bigskip
$$ \textsf{\red t may modify the global state}$$ 
 $$~$$
\infrule[T-assign] 
...
\infrule[T-Rec] 
		...
		\tau^{\textcolor{red}{1}}, {\blue \theta}, {\blue \rho}} 
...
\begin{frame}[fragile]
\frametitle{Inlining correctness}
\begin{large}
$$\hspace*{-24em} {\blue Theorem:}  $$
$$ \Gamma \vdash t: \tau^\texttt{i}, \theta, \rho \textsf{ holds }~~ {\red \boldsymbol{\Rightarrow}}~~ \textsf{   t can be inlined correctly}$$ 
$$ \hspace*{-25em} {\blue Proof:} $$
$$\hspace*{-19em} \textsf{See report...} $$  
$$ \hspace*{-24em} {\blue Example:} $$
$$\hspace*{-17em} \textsf{"let in" typing rule} $$ 
\end{large}
\end{frame}
...
${\red \only<1>{\mathcal{P}_1}\only<2>{\mathcal{P}_2}}$: 
   let F = ($\only<1>{\texttt{\red r:= !r + 1;}}\only<2>{\texttt{\red diverge(); }}$ fun (f:int -> int) -> f !r)
...
?* 
${\red\only<1>{ \mathcal{I}(P_1)}\only<2>{\mathcal{I}(P_2)}}:$ 
$\only<1>{\qquad \texttt{(r:= !r + 1; !r) + (r:= !r + 1; !r)}}
\only<2>{\qquad \texttt{()}}$ 
   
*?%$\hspace*{8em}\only<2>{\mathcal{I}(P_1)_{\{r=0\}} \longrightarrow^{\star} 3_{\{r=2\}}}\only<4>{\mathcal{I}(P_2)_{\{r=0\}} \longrightarrow^{\star} ()_{\{r=0\}}}$      
?*
\end{whycode}
\end{small} \vspace*{-2em}
\only<1>{
\begin{flushleft}
\textsf{$\bullet$ the effect is duplicated in $\mathcal{I}(P_1)$	} \\
\Rouge{$\bullet$  $P_1$ \textsf{must be rejected by type system}} 
\end{flushleft}}
\only<2>{
\begin{flushleft}
 \textsf{$\bullet$ semantics is not preserved by $\mathcal{I}(P_2)$} \\
\Rouge{$\bullet$   $P_2$ \textsf{must be rejected by type system}} 
\end{flushleft}}
\end{frame}
...
\frametitle{les liaisons moins dangereuses}
		\infrule[] 
			{\vspace*{0.5em}{ \Gamma} \vdash t_1 :  \tau^i, {\red \theta_1}, {\red \rho_1}  \phantom{tttttttt} 
			\quad { \Gamma, x:\tau^i}  \vdash t_2 :\tau^j,  {\red \theta_2}, {\red \rho_2}  \phantom{ttttt} {\red \texttt{\textbf{C}}}} 
